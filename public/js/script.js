    ...
    await array.forEach(time => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            slot.setAttribute('data-hours', time);
                ...
                // Function checks if slot with coordinates assigned above has coordinates like any one in database and if so - fullfill respectively the cell. The query method from app.js returns object (opposite to db.run from sqlite version, which returned an array) so there is a need to use Array.from
                Array.from(visitsFromDb).forEach((bookedSlot, index) => {
